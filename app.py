...
true_bht = st.number_input('true bht')


# 'pH(CaCl2)':[pH(CaCl2)],'pH(H2O)':[pH(H2O)],
if st.button('Predict Clay'):
      input=pd.DataFrame({'depth':[depth],'stemp':[stemp],'clay_present':[clay_present],'clay_type':[clay_type],'wa_or_nwa':[wa_or_nwa],'clay_content':[clay_content],'corr_bht':[corr_bht],'alpha':[alpha],'beta':[beta],'true_bht':[true_bht]})
